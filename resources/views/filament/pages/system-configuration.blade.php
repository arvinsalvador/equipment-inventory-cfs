            ...
            <x-filament::section>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    @foreach ($groupSettings as $setting)
                        <label class="block rounded-lg border border-gray-200 bg-white p-4 text-sm text-gray-900 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
                            ...
                            @elseif (in_array($setting->value_type, ['text', 'json'], true))
                                <textarea wire:model="settings.{{ $group }}.{{ $setting->key }}" rows="{{ $setting->isJson() ? 6 : 3 }}" @disabled(! $setting->isEditable()) class="mt-3 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-700 dark:bg-gray-950 dark:text-white"></textarea>
                            @else
                                <input type="{{ $this->inputType($setting) }}" wire:model="settings.{{ $group }}.{{ $setting->key }}" @disabled(! $setting->isEditable()) @if ($setting->value_type === 'decimal') step="0.01" @endif class="mt-3 block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-primary-500 focus:ring-primary-500 dark:border-gray-700 dark:bg-gray-950 dark:text-white">
                            @endif
